        ...
        <div>
            {isSuccess && (
                <div>
                        ...
                        <div>
                            {dataBoard?.data.lists.length > 0 ? (
                                <div>
                                    ...
                                    <select
                                        onChange={event => setListIndex(Number(event.target.value))}
                                        value={listIndex}
                                    >
                                        <option value={-1}>Выберите список</option>
                                        {dataBoard?.data.lists.map((list: IList, index: number) => (
                    ...
                    {isDataBoardSuccess && listIndex > -1 && dataBoard?.data.lists[listIndex] && (
                        <div>
                            {dataBoard?.data.lists[listIndex].cards.length > 0 ? (
                                <div>
                                    <h3>Выберите карточку</h3>
                                    <select onChange={event => selectCard(event.target.value)}>
